...
export default function RegisterPage() {
  ...
  useEffect(() => { fetch('/api/auth/doctors').then((r) => r.json()).then((d) => setDoctors(d.doctors || [])); }, []);

  async function submit(e: React.FormEvent) {
    e.preventDefault(); setLoading(true); setMessage('');
    const res = await fetch('/api/auth/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, email, password, doctorId, condition }) });
  ...
  return <div className="min-h-screen bg-gradient-to-br from-medical-50 via-white to-clinical-100 flex items-center justify-center p-4">
    <div className="w-full max-w-md space-y-5 page-enter">
      ...
      <form onSubmit={submit} className="card p-6 space-y-4">
        ...
        <select className="input-field" value={doctorId} onChange={(e) => setDoctorId(e.target.value)} required><option value="">Choose your clinician</option>{doctors.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</select>
